...
function pawnPromotesAtSquares(oldPosition, newPosition, oldSquare, newSquare, noOfRows, noOfColumns) {
	...
	const promotablePieces = /* @__PURE__ */ new Set([
		"Q",
		"R",
		"B",
		"N"
...
function ChessboardProvider({ children, options }) {
	...
	const drawArrow = (0, import_react.useCallback)((newArrowEndSquare, modifiers) => {
		...
		if (newArrowStartSquare && newArrowStartSquare !== newArrowEndSquare) {
			...
			if (arrowOptions.colors) {
				if (modifiers?.altKey && arrowOptions.colors.alt) arrowColor = arrowOptions.colors.alt;
				else if (modifiers?.shiftKey && arrowOptions.colors.shift) arrowColor = arrowOptions.colors.shift;
				else if (modifiers?.ctrlKey && arrowOptions.colors.ctrl) arrowColor = arrowOptions.colors.ctrl;
				else if (modifiers?.metaKey && arrowOptions.colors.meta) arrowColor = arrowOptions.colors.meta;
				else arrowColor = arrowOptions.colors.default || arrowOptions.color;
			} else arrowColor = modifiers?.shiftKey ? arrowOptions.secondaryColor : modifiers?.ctrlKey ? arrowOptions.tertiaryColor : arrowOptions.color;
	...
	const setNewArrowOverSquareWithModifiers = (0, import_react.useCallback)((square, modifiers) => {
		...
		if (arrowOptions.colors) {
			if (modifiers?.altKey && arrowOptions.colors.alt) color = arrowOptions.colors.alt;
			else if (modifiers?.shiftKey && arrowOptions.colors.shift) color = arrowOptions.colors.shift;
			else if (modifiers?.ctrlKey && arrowOptions.colors.ctrl) color = arrowOptions.colors.ctrl;
			else if (modifiers?.metaKey && arrowOptions.colors.meta) color = arrowOptions.colors.meta;
			else color = arrowOptions.colors.default || arrowOptions.color;
		} else color = modifiers?.shiftKey ? arrowOptions.secondaryColor : modifiers?.ctrlKey ? arrowOptions.tertiaryColor : arrowOptions.color;
	...
	const pointerActivationConstraint = dragActivationDistance > 0 ? { distance: dragActivationDistance } : void 0;
	const sensors = useSensors(useSensor(RightClickCancelSensor, { activationConstraint: pointerActivationConstraint }), useSensor(KeyboardSensor), useSensor(TouchSensor, { activationConstraint: pointerActivationConstraint }), useSensor(MouseSensor));
